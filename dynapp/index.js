...
const fetchData = async (user, token) => {
  ...
  const urls = [`https://api.github.com/orgs/HackYourFuture`, `https://api.github.com/orgs/HackYourFuture/repos?per_page=100`, `https://api.github.com/orgs/HackYourFuture/events`]
  ...
  const contributorsAll = await Promise.all(contributorsUrls.map(url => cachedFetch(url, options, 60 * 60).then(response => (response.status === 204 ? [] : response.json()))))
  ...
  contributorsFlattenArray.forEach(function (contributor) {
    if (!this[contributor.login]) {
      this[contributor.login] = { avatar_url: contributor.avatar_url, login: contributor.login, url: contributor.url, html_url: contributor.html_url, contributions: 0 }
  ...
  var orgreposAndContributors = orgrepos.map((value, index) => {
    value.contributors = contributorsAll[index]
    return value
  })

  return status("Abracadabra!"), drawTemplate(), drawHome(org, orgrepos, orgevents, orgusercontrs), drawMenu(org, orgrepos, orgevents, orgusercontrs, orgreposAndContributors)
}
...
<div id=activity>
  ...
  statsForks.innerHTML = `<span class="stats-number">${orgrepos.reduce((sum, { forks_count }) => sum + forks_count, 0)}</span><span class="stats-text">Spells Copied</span>`
  statsIssues.innerHTML = `<span class="stats-number">${orgrepos.reduce((sum, { open_issues_count }) => sum + open_issues_count, 0)}</span><span class="stats-text">Dangerous Spells</span>`
  ...
  statsTotalContrs.innerHTML = `<span class="stats-number">${orgusercontrs.map(item => item.contributions).reduce((prev, next) => prev + next)}</span><span class="stats-text">Total Spells Casted</span>`
  statsTotalUsers.innerHTML = `<span class="stats-number">${orgusercontrs.length}</span><span class="stats-text">Number Of Magicians</span>`
  ...
  latestActivity.innerHTML = orgevents
    .map(x => {
      ...
      'PullRequestEvent' == type &&
        ...
        <span class="action-was-made">${x.payload.action} a pull request <a href="${x.payload.pull_request.html_url}">${x.payload.pull_request.title}</a></span>        
        `)
      ...
      'IssueCommentEvent' == type &&
        (action = `<span class="action-was-made">Wrote a <a href="${x.payload.comment.html_url}">comment</a></span> in issue <a href="${x.payload.issue.html_url}">${
          x.payload.issue.title
          }</a></span>
        <span class="message"><span class="message-inner">"${x.payload.comment.body}"</span></span>`)

      'WatchEvent' == type && (action = `Starred a repo <a href="https://github.com/${x.repo.name}">${x.repo.name}</a></span><span></span>`)

      'DeleteEvent' == type && (action = `Deleted a ${x.payload.ref_type} "${x.payload.ref}" in <a href="https://github.com/${x.repo.name}">${x.repo.name}</a></span><span></span>`)

      'CreateEvent' == type &&
        (action = `Created a ${x.payload.ref_type} "${x.payload.ref}" with description: ${x.payload.description} in <a href="https://github.com/${x.repo.name}">${
          x.repo.name
          }</a></span><span></span>`)

      const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
      const orgEventTime = new Date(x.created_at)
      const time = `${orgEventTime.getDate()} ${monthNames[orgEventTime.getMonth()]} ${orgEventTime.getHours()}: ${`0${orgEventTime.getMinutes()}`.substr(-2)}`
...
function drawRepositories(orgreposAndContributors) {
  ...
  `
  let collator = new Intl.Collator(void 0, { numeric: !0, sensitivity: 'base' })
  orgreposAndContributors.sort((a, b) => collator.compare(b.forks_count, a.forks_count))
  repositoriesList = document.querySelector('.repo-list')
  repositoriesList.innerHTML = orgreposAndContributors
    .map(x => {
      let issues = ''
      let watchers = ''
      let forks = ''
      x.open_issues !== 0 && (issues = `<i class="repo-issues"><span class="repo-images-text">${x.open_issues}</span><span class="tooltiptext">Dangerous Spells</span></i>`)
      x.watchers_count !== 0 && (watchers = `<i class="repo-watchers"><span class="repo-images-text">${x.watchers_count}</span><span class="tooltiptext">Overwatchers</span></i>`)
      x.forks_count !== 0 && (forks = `<i class="repo-forks"><span class="repo-images-text">${x.forks_count}</span><span class="tooltiptext">Copies</span></i>`)
      ...
      x.contributors !== undefined &&
        (contributor = x.contributors
          .map(
            x => `
    <span class="repo-contr"><i class="repo-contr-image" style="background-image:url(${x.avatar_url})"></i><span class="repo-contr-name">${
              x.login
              }</span><i class="repo-contr-contribs">${x.contributions}<span class="tooltiptext">Spells Casted</span></i></span>`
          )
          .join(''))
...
function drawContributors(orgusercontrs) {
  ...
  usersList.innerHTML = orgusercontrs.map(x => {
    ...
    x.contributions !== 0 && (contributions = `<span class="user-images-text">${x.contributions}<i class="user-contributions"></i><span class="tooltiptext">Spells Casted</span></span>`)
    x.followers !== 0 && (followers = `<span class="user-images-text">${x.followers} <i class="user-followers"></i><span class="tooltiptext">Marionettes Owned</span></span>`)
    x.public_repos !== 0 && (public_repos = `<span class="user-images-text">${x.public_repos} <i class="user-public-repos"></i><span class="tooltiptext">White Books Owned</span></span>`)
